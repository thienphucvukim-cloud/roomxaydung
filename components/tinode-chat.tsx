...
export function TinodeChat() {
  ...
  const connect = async (event: FormEvent) => {
    ...
    try {
      ...
      const tinode = new sdk.Tinode({ appName: "TIPOOK/1.0", host, apiKey: DEFAULT_API_KEY, transport: "ws", secure: !host.includes("localhost"), persist: true });
  ...
  if (status !== "online") return <div className="mx-auto grid min-h-[calc(100vh-72px)] max-w-6xl place-items-center px-4 py-10">
    <div className="grid w-full overflow-hidden rounded-3xl border border-[#dce5ef] bg-white shadow-[0_24px_70px_rgba(11,46,89,.12)] lg:grid-cols-[1.05fr_.95fr]">
      <section className="hidden bg-gradient-to-br from-[#063a75] to-[#168ac0] p-10 text-white lg:block"><MessageCircle size={42}/><h1 className="mt-8 text-4xl font-extrabold tracking-[-.04em]">Kết nối cùng cộng đồng xây dựng</h1><p className="mt-4 max-w-md text-base leading-7 text-white/80">Trao đổi riêng tư với chủ nhà, kiến trúc sư, kỹ sư và nhà thầu ngay trong Tipook.</p><div className="mt-10 space-y-4 text-sm"><p className="flex items-center gap-3"><CheckCircle2 size={18}/>Tin nhắn được đồng bộ an toàn</p><p className="flex items-center gap-3"><CheckCircle2 size={18}/>Trạng thái trực tuyến và chưa đọc</p><p className="flex items-center gap-3"><CheckCircle2 size={18}/>Trò chuyện liền mạch trên mọi thiết bị</p></div></section>
      <section className="p-6 sm:p-10"><div className="flex items-center gap-3 text-[#0b3264]"><span className="grid size-11 place-items-center rounded-2xl bg-[#e8f6fc] text-[#168ac0]"><MessageCircle/></span><div><h2 className="text-2xl font-extrabold">{mode === "login" ? "Đăng nhập Tipook Chat" : "Tạo tài khoản Tipook Chat"}</h2><p className="text-sm text-[#667085]">Nhắn tin trực tiếp với cộng đồng</p></div></div>
