...
const plans = [
  { title: "Mặt bằng nhà phố 5×20m", subtitle: "Tầng trệt · Gia đình 4–5 người", bedrooms: "3 phòng ngủ", baths: "3 WC", feature: "Giếng trời giữa nhà" },
  { title: "Mặt bằng nhà phố 4×20m", subtitle: "3 tầng · Có phòng làm việc", bedrooms: "4 phòng ngủ", baths: "4 WC", feature: "Thông gió hai đầu" },
  { title: "Mặt bằng nhà 5×16m", subtitle: "2 tầng · Gia đình trẻ", bedrooms: "3 phòng ngủ", baths: "2 WC", feature: "Bếp nhìn ra sân sau" },
  { title: "Mặt bằng nhà 6×18m", subtitle: "3 tầng · Có gara ô tô", bedrooms: "4 phòng ngủ", baths: "4 WC", feature: "Phòng ngủ tầng trệt" },
];
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] text-[#0b2e59]">
      <main className="mx-auto max-w-[1320px] px-4 py-7 lg:px-8 lg:py-10">
        ...
        <div className="mt-6 flex gap-2 overflow-x-auto pb-2 scrollbar-none">{["Tất cả","Ngang 4m","Ngang 5m","Ngang 6m","2 tầng","3 tầng","Có gara"].map((item,index)=><button key={item} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${index===0?"bg-[#229ed9] text-white":"border border-[#e3eaf2] bg-white text-[#3f5064]"}`}>{item}</button>)}</div>
        <section className="mt-5 grid gap-5 lg:grid-cols-2">
          {plans.map((plan,index)=><article key={plan.title} className="overflow-hidden rounded-[24px] border border-[#e3eaf2] bg-white shadow-[0_5px_20px_rgba(24,49,39,.045)]">
            ...
            <div className="p-5 sm:p-6"><p className="text-sm font-bold text-[#168ac0]">{plan.subtitle}</p><h2 className="mt-1 text-xl font-extrabold tracking-[-.02em]">{plan.title}</h2><div className="mt-4 grid grid-cols-2 gap-2 text-sm font-medium text-[#3f5064]"><span className="flex items-center gap-2 rounded-xl bg-[#f7f9fc] p-3"><BedDouble size={17} className="text-[#229ed9]"/>{plan.bedrooms}</span><span className="flex items-center gap-2 rounded-xl bg-[#f7f9fc] p-3"><Bath size={17} className="text-[#229ed9]"/>{plan.baths}</span><span className="col-span-2 flex items-center gap-2 rounded-xl bg-[#eef9fd] p-3"><Sun size={17} className="text-[#147aa8]"/>{plan.feature}</span></div><div className="mt-5 flex items-center justify-between border-t border-[#e8eef5] pt-4"><button className="flex items-center gap-2 text-sm font-bold text-[#3f5064]"><Ruler size={16}/>Xem chi tiết</button><button className="flex items-center gap-1.5 rounded-lg bg-[#d9f1fb] px-3 py-2 text-sm font-bold text-[#229ed9]"><MessageCircle size={15}/>Nhờ KTS điều chỉnh</button></div></div>
          </article>)}
        </section>
                <EditorialContent
