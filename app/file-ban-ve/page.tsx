...
export default function DrawingFilesPage() {
  return <main className="mx-auto max-w-[1440px] px-4 py-7 lg:px-8 lg:py-10">
    <section className="flex flex-col justify-between gap-5 rounded-[26px] bg-[#073b74] p-7 text-white sm:p-9 lg:flex-row lg:items-center">
      ...
      <button className="flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#229ed9] px-5 py-3 font-bold"><Upload size={18}/>Đăng bán bản vẽ</button>
    </section>
    <div className="mt-7 flex flex-wrap gap-2">{["Tất cả", "Nhà phố", "Biệt thự", "Nhà cấp 4", "Công trình công cộng", "File CAD", "Hồ sơ thi công"].map((item, index) => <button key={item} className={`rounded-full px-4 py-2 text-sm font-bold ${index === 0 ? "bg-[#229ed9] text-white" : "border border-[#e3eaf2] bg-white text-[#3f5064] hover:border-[#229ed9]"}`}>{item}</button>)}</div>
    <section className="mt-6 grid gap-x-5 gap-y-9 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">{drawings.map((drawing) => <article key={drawing.title} className="group min-w-0"><div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-[#f7f9fc]"><img src={drawing.image} alt={drawing.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]"/><button className="absolute right-3 top-3 grid size-9 place-items-center rounded-full bg-white/90 text-[#3f5064] opacity-0 shadow-sm transition group-hover:opacity-100" aria-label="Lưu bản vẽ"><Heart size={17}/></button></div><div className="mt-3 flex items-center justify-between gap-3 text-xs font-semibold text-[#3f5064]"><span className="truncate text-[#147aa8]">{drawing.category}</span><span className="flex shrink-0 items-center gap-2"><span className="flex items-center gap-1"><Eye size={14}/>{drawing.views}</span><span className="flex items-center gap-1"><ArrowDownToLine size={14}/>{drawing.downloads}</span><span className="flex items-center gap-0.5 text-[#d99016]"><Star size={14} className="fill-current"/>{drawing.rating}</span></span></div><h2 className="mt-2 line-clamp-2 min-h-12 text-[15px] font-bold leading-6 text-[#1d3429]">{drawing.title}</h2><div className="mt-3 flex items-center justify-between"><strong className="text-base text-[#229ed9]">{drawing.price}</strong><button className="flex items-center gap-1 text-xs font-bold text-[#229ed9]"><ArrowDownToLine size={15}/>Xem file</button></div></article>)}</section>
        <EditorialContent
