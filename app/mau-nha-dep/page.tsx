...
const models = [
  { title: "Nhà phố 3 tầng xanh mát", meta: "5 × 20m · 4 phòng ngủ", style: "Hiện đại nhiệt đới", image: "/mau-nha-pho-xanh.png" },
  { title: "Nhà phố lệch tầng thoáng sáng", meta: "4,5 × 18m · 3 phòng ngủ", style: "Hiện đại tối giản", image: "/community-house.png" },
  { title: "Nhà 3 tầng có sân trước", meta: "5 × 16m · 4 phòng ngủ", style: "Ấm áp, gần gũi", image: "/mau-nha-pho-xanh.png" },
  { title: "Nhà phố có khoảng xanh giữa nhà", meta: "5 × 22m · 3 phòng ngủ", style: "Không gian mở", image: "/community-house.png" },
  { title: "Nhà ống 3 tầng mặt tiền lam gỗ", meta: "4 × 20m · 4 phòng ngủ", style: "Hiện đại", image: "/mau-nha-pho-xanh.png" },
  { title: "Nhà phố kết hợp kinh doanh", meta: "6 × 18m · 3 phòng ngủ", style: "Linh hoạt công năng", image: "/community-house.png" },
];
    ...
    <div className="min-h-screen bg-[#f4f7fb] text-[#0b2e59]">
      <main className="mx-auto max-w-[1320px] px-4 py-7 lg:px-8 lg:py-10">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
          ...
          <button className="flex h-11 items-center justify-center gap-2 rounded-xl border border-[#dce5ef] bg-white px-4 text-sm font-bold text-[#3f5064]"><SlidersHorizontal size={17}/>Lọc mẫu phù hợp</button>
        </div>
        <div className="mt-6 flex gap-2 overflow-x-auto pb-2 scrollbar-none">{["Tất cả","Nhà phố","Nhà vườn","Mặt tiền 5m","3 tầng","Có gara","Nhiều cây xanh"].map((item,index)=><button key={item} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${index===0?"bg-[#229ed9] text-white":"border border-[#e3eaf2] bg-white text-[#3f5064]"}`}>{item}</button>)}</div>
        <section className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {models.map((model,index)=><article key={model.title} className="group overflow-hidden rounded-[22px] border border-[#e3eaf2] bg-white shadow-[0_5px_20px_rgba(24,49,39,.045)]">
            <div className="relative aspect-[4/3] overflow-hidden"><img src={model.image} alt={model.title} className={`h-full w-full object-cover transition duration-500 group-hover:scale-[1.03] ${index%2 ? "object-center" : ""}`}/><span className="absolute left-3 top-3 rounded-lg bg-white/90 px-2.5 py-1.5 text-xs font-bold text-[#0b2e59] backdrop-blur">{model.style}</span><button className="absolute right-3 top-3 z-20 grid size-9 place-items-center rounded-full bg-white/90 text-[#3f5064] backdrop-blur" aria-label="Lưu mẫu"><Heart size={17}/></button><ProjectGallery model={model} trigger="overlay"/></div>
            <div className="p-5"><h2 className="text-lg font-extrabold tracking-[-.02em]">{model.title}</h2><p className="mt-2 flex items-center gap-2 text-sm font-medium text-[#3f5064]"><Maximize2 size={15}/>{model.meta}</p><div className="mt-5 flex items-center justify-between border-t border-[#e8eef5] pt-4"><ProjectGallery model={model}/><button className="flex items-center gap-1.5 rounded-lg bg-[#eef9fd] px-3 py-2 text-sm font-bold text-[#147aa8]"><MessageCircle size={15}/>Nhờ KTS tư vấn</button></div></div>
          </article>)}
        </section>
                <EditorialContent
        ...
        />
        <section className="mt-9 flex flex-col items-start justify-between gap-5 rounded-[24px] bg-[#073b74] p-7 text-white sm:flex-row sm:items-center sm:p-9"><div><h2 className="text-2xl font-extrabold">Chưa tìm thấy mẫu phù hợp với đất của bạn?</h2><p className="mt-2 text-white/85">Gửi kích thước đất và nhu cầu, kiến trúc sư sẽ gợi ý phương án phù hợp.</p></div><button className="flex shrink-0 items-center gap-2 rounded-xl bg-[#229ed9] px-5 py-3 font-bold">Nhận tư vấn thiết kế <ArrowRight size={18}/></button></section>
      </main>
    </div>
  );
}
